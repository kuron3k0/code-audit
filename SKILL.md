...
入口点: {Controller/Router/Handler/Filter/Interceptor/Servlet等入口的数量和列表，包括但不限于，按你的知识补充}
关键模块: {列表}
```

### Step 4: 执行计划 → STOP

基于 Step 1-3 的输出生成执行计划。
...
⚠️ 禁止跳过规则:
✗ Do NOT skip any file because "similar vulnerabilities already reported"
✗ Do NOT skip any detection command result without analyzing each line
✗ Do NOT assume a Sink is safe just because it's in a "non-critical" component

✓ MUST analyze every file independently
✓ MUST report each vulnerability even if same type exists in other files
✓ MUST run detection commands on ALL files and analyze ALL results
```
